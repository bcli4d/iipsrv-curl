// Member functions for TPTRemImage.h
...
void TPTRemImage::openImage() throw (file_error)
{
  ...
  if( ( tiff = rem_TIFFOpen( filename.c_str(), "rm" ) ) == NULL ){
...
void TPTRemImage::loadImageInfo( int seq, int ang ) throw(file_error)
{
  tdir_t current_dir;
  int count;
...
RawTile TPTRemImage::getTile( int seq, int ang, unsigned int res, int layers, unsigned int tile ) throw (file_error)
{
